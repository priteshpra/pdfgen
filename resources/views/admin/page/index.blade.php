                        ...
                        <div class="table-responsive">
                            <table id="pageTable"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <!-- @can('user_show')
                                                    ...
                                                    @endcan -->
                                            @can('user_edit')
                                            <a href="{{ route('admin.page.edit', $user->PageID) }}"
                                                class="btn btn-sm btn-warning">Edit</a>
                                            @endcan
                                            @can('user_delete')
                                            <form action="{{ route('admin.page.destroy', $user->PageID) }}"
                                    ...
                                    @empty
                                    <tr>
                                        <td colspan="100%" class="text-center text-muted py-3">No Page Found
                                        </td>
                                    </tr>
                                    @endforelse
                                </tbody>
...
<script>
    function toggleStatus(ID, status) {
        if (status == 1) {
            ...
            $('#toggleChang_'
                _ID).removeClass('btn-error');
        } else {
            statuss = 1;
            $('#toggleChang_'
                _ID).removeClass('btn-success');
            $('#toggleChang_'
                _ID).addClass('btn-error');
        }
        $("#toggleChang_"
            _ID).attr("onclick", "toggleStatus(" + ID + ", " + statuss + ")");
